...
export function loader({ page, layout }: Props) {
  const product = {
    ...
    additionalProperty: page?.product?.additionalProperty?.filter(p => p.name === "category" && p.value === "Bonés, Boinas e Gorros").map(c=> ({ name: c.name, value: c.value })) ?? [],
    isVariantOf: {
      additionalProperty: page?.product?.isVariantOf?.additionalProperty?.filter(p => p.name === "Infantil").map(c=> ({ name: c.name, value: c.value })) ?? [], 
    }
  };
