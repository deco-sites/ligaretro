...
import { ResponseReviews, Reviews, } from "$store/loaders/Reviews/reviewsandratings.ts";
...
export async function loader({ page, borderRoundedBot }: Props, _req: Request) {
...
const ReviewsList = ({ productId, reviews }: {
    productId: string;
    reviews: Reviews[];
}) => {
    return (<div>
      {reviews.map((r, i) => (<div class="grid grid-cols-5 border-t border-[#dadada] py-8">
          ...
          <div class="col-span-4 flex flex-col">
            <div class="flex justify-between mb-3">
              <div class="flex items-center gap-3">
                ...
                <RatingStars productId={`${i}-${productId}`} average={r.rating} display="reviews"/>
              </div>
...
const NoReviews = () => {
    return (<div class="flex p-5 sm:p-0 justify-center items-center bg-[#e5e5e5] mb-4 h-28">
      <h3>Este produto ainda não possui avaliações.</h3>
    </div>);
};
function ProductReviews({ page, reviews, debug, borderRoundedBot }: SectionProps<typeof loader>) {
    ...
    //   },
    // ];
    return (<section class="container my-14">
      <div class={`bg-white max-w-[90%] lg:max-w-[100%] p-5 lg:pb-5 pt-[2px] lg:p-12  border border-[#cecece]`}>
        <h3 class="uppercase my-5">
          Avaliações do produto
        </h3>
        {Boolean(reviews.data?.length) && (<div class="flex justify-start mb-6">
            <RatingStars productId={"averageReviews-" + productId} display="detailsPage" average={reviews.averageRating!.average} count={reviews.averageRating!.totalCount}/>
            {
        ...
        /* <span class="text-[#006299] cursor-pointer">
          ...
          <LoginToReview />
          <NewReviewForm productId={productId} userHasReviewed={reviews.userHasReviewed || false}/>
        </div>
      </div>
    </section>);
}
